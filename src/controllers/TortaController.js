...
export const editarTorta = async (torta) => {
  try {
    const token = await waitUntilTokenIsAvailable();
    console.log('Token utilizado para editar el torta:', token); // Agregar este console log
    console.log('Datos del torta a editar:', torta); // Agregar este console log
    const response = await axios.put(`${baseUrl}/tortas/${torta.id}`, torta, {
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
      }
    });
    console.log('Respuesta del servidor al editar torta:', response); // Agregar este console log
    return response.data;
  } catch (error) {
    console.error('Error al editar el torta:', error);
    return { success: false };
  }
};
